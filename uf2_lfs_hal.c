...
void dvReadData(struct ram_flash_sim* block_device, uint32_t address, void *buffer, lfs_size_t size) {
...
uint32_t fsAddressForBlock(struct flash_fs* fs, uint32_t block, uint32_t off) {
    ...
    return fs->device->base_address + fs->fs_device_offset + byte_offset;
}
